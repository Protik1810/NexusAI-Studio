@echo off
title Solframe Studio - Standalone Launcher
cd /d "%~dp0"
echo.
echo  Solframe Studio  v1.0.0  - Standalone AI Image + Chat Studio
echo.
where npm >nul 2>&1
if errorlevel 1 ( echo [ERROR] npm not found. Install Node.js from https://nodejs.org && pause && exit /b 1 )
echo [Solframe] Starting backend server...
start /min "" cmd /c "npm run dev"
echo [Solframe] Waiting for server to start...
set /a tries=0
:waitloop
...
:ready
echo [Solframe] Server ready! Launching app window...
...
:done
echo [Solframe] Running at http://localhost:1420 - Close this window to stop.
pause
