...
The practical consequence (see [CONTRIBUTING.md](CONTRIBUTING.md#a-note-on-the-inference-engines)): CI can build a real, installable app for every platform, but only a UI-shell one — `.github/workflows/release-build.yml` builds directly from committed source, so it has no access to these binaries and never claims to. The Windows *Complete* installer (the one with working inference) is still assembled by hand for each release, from a local copy of these engine binaries.

`scripts/fetch-engines.js` closes the "how do I even get these" half of that gap: it downloads the current release from each upstream project, verifies the SHA-256 against the checked-in `engines-lock.json`, and unpacks it into the right `backend/` subdirectory — so a fresh contributor (or a future CI job) doesn't have to know each project's release-asset naming by hand. It doesn't yet make `release-build.yml` itself build the Complete installer — wiring that up would still need a CI job with a GPU-capable runner (or at least the willingness to bundle CUDA/Vulkan binaries into a CI artifact) to actually be useful, which is a separate decision from just being able to fetch the binaries.
